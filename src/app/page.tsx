...
export default function KaeliaNexusV5() {
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-zinc-100 font-sans selection:bg-pink-500/30">
        ...
        <div className="max-w-[1800px] mx-auto flex items-center justify-between gap-6">
          ...
          <div className="flex-1 max-w-xl relative">
            <input 
              ...
              placeholder={search.startsWith('@') ? `Viendo a ${search}` : "Buscar recuerdos..."} 
              className="w-full bg-white/5 border border-white/10 p-3 px-6 rounded-2xl outline-none focus:border-pink-500/50" 
            />
            {search && <button onClick={() => setSearch("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-pink-500 uppercase">Limpiar</button>}
          </div>
      ...
      <div className={`fixed inset-y-0 left-0 w-80 bg-[#050505] border-r border-white/5 z-[400] transition-transform duration-500 p-8 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {currentUser && (
          <div className="space-y-4">
            ...
            <input value={upCover} onChange={e => setUpCover(e.target.value)} placeholder="URL Portada" className="w-full bg-white/5 p-4 rounded-xl text-xs border border-white/5 outline-none" />
      ...
      <div className={`fixed inset-y-0 right-0 w-85 bg-[#050505] border-l border-white/10 z-[400] transition-transform duration-500 p-10 ${isUserPanelOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center mb-10">
          <span className="text-[10px] font-black text-zinc-500 uppercase">Perfil</span>
        ...
        {currentUser ? (
          <div className="space-y-8">
            ...
            <div className="space-y-3">
              <button onClick={() => {const n = prompt("Nueva URL Avatar:"); if(n) updateDoc(doc(db, "users", currentUser.user), {pfp: n});}} className="w-full bg-white/5 p-4 rounded-xl text-[10px] font-black uppercase text-left">Cambiar Foto</button>
              <button onClick={() => {const n = prompt("Nuevo PIN:"); if(n) updateDoc(doc(db, "users", currentUser.user), {pass: n});}} className="w-full bg-white/5 p-4 rounded-xl text-[10px] font-black uppercase text-left">Cambiar PIN</button>
      ...
      <main className="p-8 max-w-[1900px] mx-auto">
        {search.startsWith('@') && (
          <div className="mb-10 flex items-center gap-4">
            ...
            <h2 className="text-3xl font-black italic uppercase">@{search.slice(1)}</h2>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-10">
          {filteredVideos.map(vid => (
            <div key={vid.id} className="group relative">
              ...
              <div className={`relative z-10 flex flex-col h-full rounded-[35px] overflow-hidden border transition-all bg-[#0a0a0a] ${vid.featured ? 'border-yellow-500/50' : 'border-white/5 hover:border-pink-500/30'}`}>
                <div onClick={() => setSelectedVid(vid)} className="cursor-pointer">
                  ...
                  <div className="p-6">
                    <h3 className={`font-black text-sm truncate uppercase italic mb-3 ${vid.featured ? 'text-yellow-500' : 'text-white'}`}>{vid.title}</h3>
                    
                    <div className="flex gap-3 items-center mb-4" onClick={(e) => {e.stopPropagation(); setSearch('@' + vid.uploader);}}>
                      <img src={users[vid.uploader]?.pfp} className={`w-7 h-7 rounded-lg object-cover border ${vid.featured ? 'border-yellow-500/50' : 'border-white/10'}`} />
                      <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest hover:text-pink-500">@{vid.uploader}</p>
                    </div>

                    {/* TAGS VISIBLES EN EL FEED */}
                    <div className="flex flex-wrap gap-1">
                      {vid.tags?.slice(0, 3).map((t:any) => (
                        <span key={t} className="text-[7px] bg-white/5 border border-white/5 px-2 py-0.5 rounded-md text-zinc-500 font-black uppercase">#{t}</span>
                      ))}
                    </div>
                  </div>
                </div>
          ...
          <div className="max-w-[1700px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
            <div className="lg:col-span-8 space-y-6">
              <div className="flex justify-between items-center">
                <button onClick={() => setSelectedVid(null)} className="text-[10px] font-black uppercase bg-white/5 px-6 py-3 rounded-xl">← Volver</button>
                {(isAdmin || currentUser?.user === selectedVid.uploader) && (
                  <div className="flex gap-2">
                    ...
                    {isAdmin && <button onClick={() => updateDoc(doc(db, "videos", selectedVid.id), {featured: !selectedVid.featured})} className="bg-yellow-500 text-black p-3 rounded-xl text-[10px] font-black">CORONA</button>}
              ...
              <div className="p-8 bg-white/[0.02] rounded-[40px] border border-white/5">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h2 className="text-4xl font-black italic uppercase text-pink-500 mb-2">{selectedVid.title}</h2>
                    <div className="flex items-center gap-4 cursor-pointer" onClick={() => {setSearch('@' + selectedVid.uploader); setSelectedVid(null);}}>
                      <img src={users[selectedVid.uploader]?.pfp} className="w-12 h-12 rounded-2xl border-2 border-pink-500/30 object-cover" />
                      <div>
                        <p className="text-xs font-black uppercase text-zinc-400">@{selectedVid.uploader}</p>
                        <p className="text-[9px] text-zinc-600 uppercase font-bold tracking-[2px]">Ver Perfil</p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              <h3 className="text-pink-500 font-black text-[10px] uppercase tracking-[6px] italic ml-2">Sugerencias</h3>
      ...
      {editVid && (
        <div className="fixed inset-0 bg-black/95 z-[1000] flex items-center justify-center p-6 backdrop-blur-xl">
          <div className="bg-[#111] p-10 rounded-[45px] border border-white/10 w-full max-w-lg space-y-5">
            <h2 className="text-xl font-black uppercase italic text-pink-500 text-center">Editar Recuerdo</h2>
            <input value={editVid.title} onChange={e => setEditVid({...editVid, title: e.target.value})} placeholder="Título" className="w-full bg-black p-4 rounded-xl border border-white/5 text-xs outline-none focus:border-pink-500" />
            <input value={editVid.cover} onChange={e => setEditVid({...editVid, cover: e.target.value})} placeholder="Portada URL" className="w-full bg-black p-4 rounded-xl border border-white/5 text-xs outline-none focus:border-pink-500" />
            <input value={editVid.tags?.join(',')} onChange={e => setEditVid({...editVid, tags: e.target.value.split(',')})} placeholder="Tags (coma)" className="w-full bg-black p-4 rounded-xl border border-white/5 text-xs outline-none focus:border-pink-500" />
            <div className="flex gap-4 pt-4">
              <button onClick={() => setEditVid(null)} className="flex-1 p-4 bg-zinc-800 rounded-xl font-black text-[10px] uppercase">Cerrar</button>
              <button onClick={async () => {
              ...
              }} className="flex-1 p-4 bg-pink-600 rounded-xl font-black text-[10px] uppercase">Guardar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAdminModal && (
        <div className="fixed inset-0 bg-black/95 z-[2000] flex items-center justify-center">
          <div className="bg-[#111] p-12 rounded-[50px] border border-white/10 text-center">
            <h2 className="text-white font-black uppercase tracking-[5px] mb-8 italic">Master Key</h2>
